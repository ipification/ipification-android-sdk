...
/**
 * Adds the SDK, device and carrier headers to the initial IP request and applies the configured
 * browser-style headers to every redirect hop.
 *
 * Redirect termination is handled separately by [HandleRedirectInterceptor].
 */
class SdkHeadersInterceptor(
    context: Context,
    ...
    /** Adds SDK, host app and optional carrier information to the initial request. */
    ...
    private fun log(message: String) {
        if (IPConfiguration.getInstance().debug) {
            IPLogs.getInstance().LOG +=
                "${LogUtils.currentTimestamp()} - SdkHeadersInterceptor - $message\n"
        }
    }
}
