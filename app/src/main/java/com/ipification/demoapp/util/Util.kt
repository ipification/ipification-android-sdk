...
class Util {
    ...
    companion object Factory {
        ...
        private fun showAlert(context: Context, title: String? = "", message: String?): AlertDialog {
...
//            .setNegativeButton(android.R.string.no, null)
        ...
        fun isCellularInternetAvailable(context: Context): Boolean {
            val cm = context.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager

            // For Android M (API 23) and above, inspect all networks
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                // Legacy path for pre-Marshmallow
                @Suppress("DEPRECATION")
                val activeInfo: NetworkInfo? = cm.activeNetworkInfo
                @Suppress("DEPRECATION")
                if (activeInfo != null
